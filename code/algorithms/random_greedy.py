...
def greedy_route(railmap, max_duration):
    """
    Generating one route using the greedy approach, prioritizing unvisited connections
...
def random_greedy_algorithm(stations_path, uid_path, connections_path, num_routes, max_duration, iterations):
    """
    Running the random greedy algorithm multiple times and returning the final quality score.
    """
    railmap = Railmap()
